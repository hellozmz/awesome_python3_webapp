...
def api_create_comment(id, request, *, content):
    ...
    #comment = Comment(blog_id=blog.id, user_id=user.id, user_name=user.name, user_image='timg.jpg', content=content.strip())
...
@post('/api/blogs')
def api_create_blog(request, *, name, private_blogs, summary, content):
    check_admin(request)
    if not name or not name.strip():
...
@get('/welcome/{name}')
def welcome(*, name,request):
    text = '<h1>hello %s!</h1>' %request.match_info['name']
    #ans.content_type = 'text/html'
    #ans.body=text.encode('utf-8')
    #return ans
    #return web.Response(body=b"text.encode('utf-8')")
